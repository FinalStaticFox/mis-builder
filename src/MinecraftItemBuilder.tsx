...
export default function MinecraftItemBuilder() {
  ...
  useEffect(() => {
    fetch("items.json?v=" + Date.now()) // TODO: Is putting the current date in the URL really necessary?
      ...
      .catch((error) => {
        ...
        alert(
          "Failed to load items.json. Make sure the file exists in the same folder."
        );
        setLoading(false);
      });
  }, []);
  ...
  const tagSuggestions = useMemo(() => {
    ...
    catalogueItems.forEach((item) => {
      ...
      if (item.material) {
        materialCounts[item.material] =
          (materialCounts[item.material] || 0) + 1;
      }
    });
  ...
  const availableItems = useMemo(() => {
    ...
    if (materialFilter) {
      ...
      filtered = filtered.filter(
        (item) => item.material && item.material.toLowerCase() === matLower
      );
    }

    // Apply stack size filter
    const activeStackSizes = Object.keys(stackSizeFilters)
      .filter((size) => stackSizeFilters[size])
      .map(Number);
    if (activeStackSizes.length > 0) {
      filtered = filtered.filter(
        (item) => item.stack_size && activeStackSizes.includes(item.stack_size)
      );
    }

    // Apply rarity filter
    const activeRarities = Object.keys(rarityFilters)
      .filter((size) => rarityFilters[size])
      .map(String);
    if (activeRarities.length > 0) {
      filtered = filtered.filter(
        (item) => item.rarity && activeRarities.includes(item.rarity)
      );
    }
    ...
    if (statsSortMode !== "none" && playerStats) {
      ...
      if (sortedItems.length > 0) {
        ...
        const mapPercentile = (linearPercent: number) => {
          ...
          const k = 30;
          const normalized =
            (Math.exp(linearPercent / k) - 1) / (Math.exp(100 / k) - 1);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (importData.items && Array.isArray(importData.items)) {
          ...
          alert(
            "Invalid file format: Expected Litematica format or array of cells"
          );
          return;
        }
          ...
          cells.map((cell) => {
            if (cell.id === cellId) {
              ...
              if (isSameCell) {
                ...
                if (
                  isTargetAfterSelection &&
                  !selectedIndices.includes(slotIndex) &&
  ...
  const findBestSplitPoint = (items: Item[]) => {
    ...
    const normalized = items.map((item) => {
      const { normalized: normId, replacements } = normalizeId(
        item.id,
        compoundTokens
      );
  ...
  const sortCellByNamespace = (cellId: number) => {
    ...
    setCells(
      cells.map((cell) => {
        ...
        const firstKey =
          splitItemId(items[0], splitPoint).prefix || "_no_prefix";
  ...
  const sortCellByType = (cellId: number) => {
    ...
    setCells(
      cells.map((cell) => {
        ...
        const firstKey =
          splitItemId(items[0], splitPoint).suffix || "_no_suffix";
        const firstGroup = groups[firstKey];
  ...
  const substituteAdjustedItem = (
    ...
    setCells(
      cells.map((cell) => {
        ...
        console.log(
          "Target:",
          targetDisplayItem?.id,
          "isAdjusted:",
          targetDisplayItem?.isAdjusted,
          "stack_size:",
          targetDisplayItem?.stack_size
        );
        console.log(
          "Source:",
          sourceDisplayItem?.id,
          "isAdjusted:",
          sourceDisplayItem?.isAdjusted,
          "stack_size:",
          sourceDisplayItem?.stack_size
        );
        ...
        console.log(
          "  Target item in newSlots[" + targetActualIndex + "]:",
          newSlots[targetActualIndex]?.id
        );
        console.log(
          "  Source item in newSlots[" + sourceActualIndex + "]:",
          newSlots[sourceActualIndex]?.id
        );
        ...
        if (sourceDisplayItem && sourceDisplayItem.isAdjusted) {
          ...
          console.log(
            ...
            sourceDisplayItem.isAdjusted,
            "quantity:",
            sourceDisplayItem.quantity
          );
          console.log(
            "Target isAdjusted:",
            targetDisplayItem.isAdjusted,
            "quantity:",
            targetDisplayItem.quantity
          );
          ...
          if (sourceDisplayItem.id === targetDisplayItem.id) {
            ...
            if (stackSize > 1) {
              ...
              console.log(
                "After compaction, slot count:",
                newSlots.filter((s) => s !== null).length
              );
            }
          ...
          console.log(
            "Before duplication - total items:",
            newSlots.filter((s, i) => i > 0 && s !== null).length
          );
          ...
          console.log(
            "Base source (with forceAdjusted):",
            baseSource.id,
            baseSource.forceAdjusted
          );
          console.log(
            "Base target (no forceAdjusted):",
            baseTarget.id,
            baseTarget.forceAdjusted
          );
          ...
          console.log(
            "Items with forceAdjusted:",
            newSlots.filter((s, i) => i > 0 && s !== null && s.forceAdjusted)
              .length
          );
          ...
          console.log(
            "Insert displaced target at first null position:",
            insertPosition
          );
        ...
        // Case 2: Source is unadjusted
          ...
          const targetCount = newSlots.filter(
            (s) => s && s.id === targetId
          ).length;
          ...
          if (targetCount > 1) {
            ...
            console.log(
              ...
              newSlots[targetActualIndex].id,
              "forceAdjusted:",
              newSlots[targetActualIndex].forceAdjusted
            );
            console.log(
              "After swap - position",
              sourceActualIndex,
              ...
              newSlots[sourceActualIndex].id,
              "forceAdjusted:",
              newSlots[sourceActualIndex].forceAdjusted
            );
            console.log(
              "Total items with forceAdjusted:",
              newSlots.filter((s, i) => i > 0 && s && s.forceAdjusted).length
            );
          }
        }
                ...
                <div className="grid grid-cols-3 gap-2">
                  {availableItems.map((item) => (
                    <div
                      ...
                      draggable
                      onDragStart={(e) =>
                        handleItemDragStart(e, item, { type: "catalogue" })
                      }
                    ...
                    >
                      ...
                      />
                      <div className="text-xs text-center mt-1 truncate pointer-events-none">
                        {item.name}
                      </div>
                    </div>
                  ))}
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="bg-stone-900 border-b-2 border-stone-700 p-4">
              <div className="flex items-center gap-4">
                ...
                {/* Import Button */}
                <label
                  className="px-3 py-1.5 bg-green-700 hover:bg-green-600 rounded text-xs transition-colors whitespace-nowrap cursor-pointer"
                  title="Import cells from JSON"
                >
                  Import
                  <input
                    type="file"
                ...
                <button
                  ...
                  title={
                    displayMode
                      ? "Redstone Mode: Optimized for signal strength"
                      : "Normal Mode"
                  }
                >
                    ...
                    <div className="flex flex-col gap-3 min-w-[200px] overflow-visible">
                      ...
                      <div className="relative z-50">
                        ...
                        {showCategorySuggestions &&
                          !tagSuggestions.categories.some(
                            (cat) =>
                      ...
                      <div className="relative z-40">
                        ...
                        {showGroupSuggestions &&
                          !tagSuggestions.groups.some(
                            (grp) =>
                              grp.toLowerCase() === groupFilter.toLowerCase()
                          ) && (
                            <div className="absolute z-50 w-full mt-1 bg-stone-700 border border-stone-600 rounded shadow-lg max-h-96 overflow-y-auto">
                      ...
                      <div className="relative z-30">
                        ...
                        {showMaterialSuggestions &&
                          !tagSuggestions.materials.some(
                            (mat) =>
                              mat.toLowerCase() === materialFilter.toLowerCase()
                          ) && (
                            <div className="absolute z-50 w-full mt-1 bg-stone-700 border border-stone-600 rounded shadow-lg max-h-96 overflow-y-auto">
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <label
                              htmlFor="stats-import"
                              className="px-3 py-1.5 bg-blue-700 hover:bg-blue-600 rounded text-xs transition-colors whitespace-nowrap cursor-pointer"
                            >
                        ...
                        {statsSortMode !== "none" && playerStats && (
                          <div className="flex flex-col items-center gap-1">
                            ...
                            >
                              ...
                              <div
                                ...
                                style={{
                                  ...
                                  bottom: `${
                                    statsPercentRange[0] === null
                                      ? 0
                                      : (statsPercentRange[0] / 100) * 128 + 12
                                  }px`,
                                }}
                              ></div>
                              ...
                              {/* Slider track */}
                              <div
                                className="relative w-6 flex flex-col"
                                style={{ height: "140px" }}
                              >
                                {/* 0-100 range (128px) */}
                                <div
                                ...
                                {/* NA block (12px) */}
                                <div
                                  className="relative rounded-b overflow-hidden bg-gray-700"
                                  style={{ height: "12px" }}
                                >
              ...
              <div className="flex flex-wrap gap-6">
                {cells.map((cell) => {
                  ...
                  const displaySlots = displayMode
                    ...
                    : cell.slots;
                  const signalStrength = displayMode
                    ? calculateSignalStrength(displaySlots)
                    : null;
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        onDragStart={(e) =>
                          !displayMode && handleCellDragStart(e, cell.id)
                        }
                      ...
                      >
                        {displaySlots.map((slot, slotIndex) => {
                          ...
                          const isSelected = selectedSlots.some(
                            (s) =>
                              s.cellId === cell.id && s.slotIndex === slotIndex
                          );
                          ...
                          return (
                            <div
                              ...
                              onDragEnd={
                                !isReserved ? handleItemDragEnd : undefined
                              }
                              onDragOver={(e) =>
                                !isReserved &&
                                handleItemDragOver(e, cell.id, slotIndex)
                              }
                              onDragLeave={
                                !isReserved ? handleItemDragLeave : undefined
                              }
                              onDrop={(e) =>
                                !isReserved &&
                                handleItemDrop(e, cell.id, slotIndex)
                              }
                              onClick={(e) => {
                              ...
                              className={`w-12 h-12 border rounded flex items-center justify-center transition-colors relative group ${
                                isReserved
                                  ...
                                  ? `${
                                      isDragOver && isAdjusted
                                        ...
                                        : `bg-stone-700 ${
                                            isAdjusted
                                              ? "border-2 border-red-500 shadow-red-500/50 shadow-md"
                                              : "border-stone-600"
                                          }`
                                    } ${
                                      slot
                                  ...
                                  : `${
                                      isDragOver
                                        ...
                                        : `bg-stone-700 ${
                                            isSelected
                                              ? "border-2 border-blue-400 bg-blue-900 bg-opacity-30"
                            ...
                            >
                              {isReserved ? (
                                <div className="text-stone-700 text-xs pointer-events-none">
                                  —
                                </div>
                              ) : slot ? (
                                <>
                                ...
                                <div className="text-stone-600 text-xs pointer-events-none">
                                  +
                                </div>
                              )}
                            </div>
